#!/usr/bin/env python3
"""
bear_lint_single.py - Markdown lint/fix engine, used by the "Bear Lint (One Note)" Shortcut.

Reads a note's Markdown from stdin, writes the fixed Markdown to stdout.
A short issue report goes to stderr, kept separate from stdout on purpose
so the Shortcut can pipe stdout straight back into Bear without the report
text ending up inside the note.

This script only transforms text. It never touches Bear, the filesystem,
or the network. Finding the note and writing the result back is handled
by Bear's native "Search Notes", "Get Note", and "Add Text to Note"
Shortcuts actions, a Run Shell Script step in between calls this script.
See README.md for the full Shortcut setup.

USAGE
  cat note.md | python3 bear_lint_single.py > fixed.md
  python3 bear_lint_single.py --selftest   # sanity-check the rules, no input needed
...
def main():
    if "--selftest" in sys.argv:
        ...
        print("=== bear_lint_single.py selftest ===", file=sys.stderr)
        print_report(issues)
